...
export async function spendXp(memberId: any, xp: any, guildId: any) {
	const cost = Math.max(0, Math.floor(Number(xp) || 0));
	...
	const updated = await db.updateMemberLevelStats(memberId, { xpIncrement: -cost });
