...
export const modules = [
	...
	},
	{
		...
		fields: [
			...
			{
				label: 'Themes',
				desc: 'Always on, nothing to configure. Each member uploads a background image (PNG, JPG, GIF or WEBP) on their account Themes tab. Everything is re-encoded and resized to WebP — animated GIFs become animated WebP, keeping their frames and looping — and the stored result must land under 1MB. The accent colour is read from the image and can be overridden by hand. Animated effects — glitch, sparkle, snow, rain, embers, confetti, bubbles, scanlines, grain, holo, aurora or pulse — are won by spinning a reel for 1,000 XP on the Themes tab, and each spin also rolls a one-of-a-kind variant, so no two members look the same. An effect can be disabled and re-enabled without spinning again, so turning it off never costs the XP already paid. Spins post to the minigames channel and appear in the member’s History, but are excluded from the Minigames leaderboard because they are a fixed-price roll, not a wager. All of it repaints that member’s account pages, wallet card, shareable card, their row on the leaderboard and their card in the members list, so every visitor sees it. Effects never appear in the downloaded card image. Themes are per server, and a disguised member is already hidden from the public leaderboard.'
			},
