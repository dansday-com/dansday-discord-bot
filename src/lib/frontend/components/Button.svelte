<script lang="ts">
	...
	const base =
		'inline-flex items-center justify-center gap-[7px] rounded-[10px] font-semibold whitespace-nowrap no-underline cursor-pointer border-0 transition-transform hover:-translate-y-px active:translate-y-0';
	const size = $derived(compact ? 'px-3.5 py-1.5 text-base' : 'px-[18px] py-[9px] text-base');
	const look = $derived(
		variant === 'primary'
