...
{#if open}
	<div
		...
		tabindex="-1"
	>
		<div class="bg-ash-800 border-ash-700 my-4 w-full max-w-md rounded-2xl border p-4 lg:p-6">
			<div class="mb-4 flex items-center justify-between lg:mb-5">
				<h3 class="text-ash-100 flex items-center gap-2 text-base font-bold lg:text-lg">
					{#if dangerous}
			...
			<p class="text-ash-200 mb-5 text-base lg:text-base">{message}</p>
			...
			<div class="flex gap-2 lg:gap-3">
				<button
					...
					onclick={oncancel}
					disabled={loading}
					class="bg-ash-700 hover:bg-ash-600 text-ash-100 flex-1 rounded-lg py-2.5 text-base font-medium transition-colors disabled:opacity-50 lg:py-3 lg:text-base"
				>
					{cancelLabel}
				</button>
				<button
