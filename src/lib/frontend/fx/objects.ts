...
export function makeMoney(rows: number): FxProgram {
	return {
		...
		init(s) {
			...
			(s as any).id = { note: 0.3 + r() * 0.4, gold: r() * 6.28, lean: (r() - 0.5) * 0.6, coinOdds: 0.1 + r() * 0.18 };
			(s as any).pile = new Float32Array(s.w);
			(s as any).coins = [] as number[][];
			(s as any).next = 20 + ((s.rnd() * 60) | 0);
			for (let i = 0; i < s.n; i++) {
				const o = i * P;
				s.parts[o] = s.rnd() * s.w;
				s.parts[o + 1] = -s.rnd() * s.h * 1.6;
				s.parts[o + 2] = 0.22 + s.rnd() * 0.3;
		...
		frame(s) {
			clear(s);
			const id = (s as any).id as { note: number; gold: number; lean: number; coinOdds: number };
			...
			for (let i = 0; i < s.n; i++) {
				const o = i * P;
				const rest = s.h - 1 - pile[Math.max(0, Math.min(s.w - 1, p[o] | 0))];
				p[o + 3] += 0.07 * p[o + 4];
				p[o] += Math.sin(p[o + 3]) * 0.36 * s.v.drift + id.lean * 0.14 * s.v.dir;
				...
				if (p[o + 1] >= rest) {
					...
					for (let k = -3; k <= 3; k++) {
						...
						pile[c] = Math.min(s.h * 0.42, pile[c] + (k === 0 ? 0.5 : 0.34 - Math.abs(k) * 0.07));
					}
				...
				for (let dy = -hh; dy <= hh; dy++) {
					for (let dx = -hw; dx <= hw; dx++) {
						...
						const cg = mid ? ig : ng;
						const cb = mid ? ib : nb;
						paint(s, p[o] + dx, p[o + 1] + dy, cr * sh, cg * sh, cb * sh, (rim ? 0.72 : 0.95) * fade);
					}
				}
			...
			if (--(s as any).next <= 0) {
				(s as any).next = 40 + ((s.rnd() * 150) | 0);
				coins.push([s.rnd() * s.w, -3, 0.3 + s.rnd() * 0.4, 0, s.rnd() * 6.28]);
				if (coins.length > 8) coins.shift();
			}
			for (let k = coins.length - 1; k >= 0; k--) {
				...
				if (c[1] >= rest) {
					if (c[3] < 1) {
						...
						pile[col] = Math.min(s.h * 0.42, pile[col] + 0.6);
						coins.splice(k, 1);
						continue;
					}
				}
			...
			for (let x = 0; x < s.w; x++) {
				...
				for (let k = 0; k < h; k++) {
					...
					const key = (x * 31 + k * 17) | 0;
					const grit = chip(x % 16, k % 16, key) / 100;
			...
			s.out = Math.min(1, stacked / (s.w * s.h * 0.05));
			blit(s);
		}
	};
}
...
export function makeCat(rows: number): FxProgram {
	return {
		...
		frame(s) {
			...
			for (let y = 0; y < s.h; y++) {
				const f = y / s.h;
				for (let x = 0; x < s.w; x++) paint(s, x, y, fr * (0.7 + f * 0.5), fg * (0.7 + f * 0.5), fb, 0.26 + f * f * 0.52);
			}
			for (let x = 0; x < s.w; x++) for (let y = Math.round(floorY); y < s.h; y++) paint(s, x, y, fr * 1.5, fg * 1.5, fb * 1.4, 0.9);
			...
			const bodyY = floorY - s.h * (settled ? 0.12 : 0.1) - Math.abs(stride) * 0.3;
			...
			const bw = s.w * 0.1;
			const bh = s.h * (settled ? 0.16 : 0.12);
			for (let dy = -bh; dy <= bh; dy++) {
				for (let dx = -bw; dx <= bw; dx++) {
					...
					const lit = 0.74 + Math.max(0, -dy / bh) * 0.4;
			...
			const hx = cx + bw * 0.86 * dir;
			const hy = bodyY - bh * (settled ? 0.86 : 0.62);
			const hr2 = s.h * 0.075;
			...
			const flick = Math.sin(s.t * 0.05 * s.v.speed) * 0.9 + Math.sin(s.t * 0.11) * 0.4;
			...
			s.out = Math.min(1, Math.abs(flick) * 0.6 + (shut ? 0.4 : 0));
			blit(s);
		}
	};
}
...
export function makeCoffee(rows: number): FxProgram {
	return {
		...
		frame(s) {
			...
			s.out = Math.min(1, Math.max(0, (steam - 2.5) / 16));
			blit(s);
		}
	};
}
...
export function makeCandle(rows: number): FxProgram {
	return {
		...
		init(s) {
			...
			(s as any).id = { wax: 0.16 + r() * 0.1, lean: (r() - 0.5) * 0.4, burn: 900 + ((r() * 500) | 0), drips: r() * 6.28 };
		...
		frame(s) {
			clear(s);
			const id = (s as any).id as { wax: number; lean: number; burn: number; drips: number };
			...
			for (let y = topY; y < base; y++) {
				...
				for (let x = cx - halfW; x <= cx + halfW; x++) {
					const u = (x - cx) / halfW;
					const lit = 0.62 + (1 - Math.abs(u + 0.25)) * 0.52;
			...
			const sway = Math.sin(s.t * 0.06 * s.v.speed) * 0.5 + Math.sin(s.t * 0.17) * 0.3;
			for (let k = 0; k < 5; k++) {
			...
			for (let dy = -8; dy <= 8; dy++) {
				for (let dx = -10; dx <= 10; dx++) {
					...
					if (d > 1) continue;
					plot(s, cx + dx, wickY + dy, fr2, fg2, fb2, (1 - d) * (1 - d) * 0.16);
				}
			}

			s.out = Math.min(1, Math.max(0, (glow - 22) / 3.4));
			blit(s);
		}
	};
}
...
export function makeVinyl(rows: number): FxProgram {
	return {
		...
		frame(s) {
			...
			s.out = Math.min(1, 0.35 + Math.abs(lit) * 0.6);
			blit(s);
		}
	};
}
...
export function makePopcorn(rows: number): FxProgram {
	return {
		...
		init(s) {
			...
			(s as any).id = { pan: 0.2 + r() * 0.1, heat: 0.6 + r() * 0.7, tint: r() };
			(s as any).pile = new Float32Array(s.w);
			for (let i = 0; i < s.n; i++) {
				const o = i * P;
		...
		frame(s) {
			clear(s);
			const id = (s as any).id as { pan: number; heat: number; tint: number };
			const pile = (s as any).pile as Float32Array;
			const p = s.parts;
			const [br, bg, bb] = hsl(s.v.hue2, s.v.sat * 0.3, 8);
			...
			const heatGlow = 0.5 + 0.5 * Math.sin(s.t * 0.06 * s.v.speed);
			for (let x = cx - panW; x <= cx + panW; x++) plot(s, x, panY + 4, 255, 120, 40, heatGlow * 0.4);

			let popped = 0;
			for (let i = 0; i < s.n; i++) {
				const o = i * P;
				if (p[o + 4] < 1) {
					p[o + 4] += 0.0016 * id.heat * s.v.speed * (0.6 + s.rnd() * 0.8);
					const rr = 1.1;
					for (let dy = -rr; dy <= rr; dy++)
						for (let dx = -rr; dx <= rr; dx++) {
							if (dx * dx + dy * dy > rr * rr) continue;
							const hot = p[o + 4];
							paint(s, p[o] + dx, panY - 1 + dy, kr * (1 + hot * 0.5), kg * (1 - hot * 0.2), kb * (1 - hot * 0.4), 0.95);
						}
					if (p[o + 4] >= 1) {
						p[o + 2] = (s.rnd() - 0.5) * 1.6;
						p[o + 3] = -1.3 - s.rnd() * 1.2;
...
export function makeClock(rows: number): FxProgram {
	return {
		...
		init(s) {
			...
			(s as any).id = { gears, brass: r(), pend: 0.6 + r() * 0.5 };
		},
		frame(s) {
			clear(s);
			const id = (s as any).id as { gears: number[][]; brass: number; pend: number };
			const [br, bg, bb] = hsl(s.v.hue2, s.v.sat * 0.3, 8);
			...
			for (let y = 0; y < s.h; y++) {
				const f = y / s.h;
				for (let x = 0; x < s.w; x++) paint(s, x, y, br, bg, bb, 0.26 + f * f * 0.48);
			}
			...
			for (let gi = 0; gi < id.gears.length; gi++) {
				...
				const rot = (s.t * 0.012 * s.v.speed * s.v.dir * dirg * 12) / teeth + g[4];
			...
			const swing = Math.sin(s.t * 0.03 * s.v.speed * id.pend) * 0.5;
			...
			s.out = Math.min(1, 0.3 + Math.abs(Math.sin(swing)) * 0.7);
			blit(s);
		}
	};
}
...
export function makeRainglass(rows: number): FxProgram {
	return {
		...
		init(s) {
			...
			(s as any).id = { fogT: 0.3 + r() * 0.4, tilt: (r() - 0.5) * 0.3, rate: 0.4 + r() * 0.8 };
		...
		frame(s) {
			clear(s);
			const id = (s as any).id as { fogT: number; tilt: number; rate: number };
			...
			for (let y = 0; y < s.h; y++) {
				const f = y / s.h;
				for (let x = 0; x < s.w; x++) {
					const glowA = Math.exp(-Math.hypot((x - s.w * 0.28) / (s.w * 0.3), (y - s.h * 0.3) / (s.h * 0.4)));
					paint(s, x, y, br + glowA * 70, bg + glowA * 60, bb + glowA * 40, 0.4 + f * 0.4);
				}
			}
			...
			for (let y = 0; y < s.h; y++) {
				for (let x = 0; x < s.w; x++) {
					...
					paint(s, x, y, fr2, fg2, fb2, mist * id.fogT * 0.72);
				}
			}

			if (--(s as any).next <= 0) {
				(s as any).next = Math.max(2, (8 / id.rate) | 0) + ((s.rnd() * 10) | 0);
			...
			s.out = Math.min(1, drops.length / 22);
			blit(s);
		}
	};
}
...
export function makeDice(rows: number): FxProgram {
	return {
		...
		init(s) {
			...
			(s as any).id = { dice, felt: r(), throwEvery: 260 + ((r() * 220) | 0) };
			(s as any).settled = 0;
		},
		frame(s) {
			clear(s);
			...
			for (let y = 0; y < s.h; y++) {
				...
				for (let x = 0; x < s.w; x++) {
					...
					paint(s, x, y, br * (0.86 + nap * 0.28), bg * (0.86 + nap * 0.28), bb, 0.42 + f * f * 0.5);
				}
			}
