-- Migration 0001: Replace server_discord_quests with bot_discord_quests catalog + server_discord_quests join table
...
SET @old_col_exists := (
    ...
    WHERE TABLE_SCHEMA = DATABASE()
      AND TABLE_NAME = 'server_discord_quests'
      AND COLUMN_NAME = 'discord_quest_id'
);
SET @sql_migrate := IF(
    @old_col_exists > 0,
    'INSERT IGNORE INTO bot_discord_quests (bot_id, quest_id, quest_task_type, quest_task_label, quest_name, game_title, quest_url, quest_description, orb_hint, rewards_line, task_detail_line, starts_at, expires_at, notified_at) SELECT 0, discord_quest_id, quest_task_type, quest_task_label, quest_name, game_title, quest_url, quest_description, orb_hint, rewards_line, task_detail_line, starts_at, expires_at, notified_at FROM server_discord_quests',
    'SELECT 1'
);
...
-- 3. Drop FK on server_discord_quests.server_id before dropping the table
SET @fk_sdo_server := (
...
SET @sql_drop_fk_sdo_server := IF(
    ...
    'SELECT 1',
    CONCAT('ALTER TABLE server_discord_quests DROP FOREIGN KEY ', @fk_sdo_server)
);
...
-- 4. Drop old server_discord_quests table
DROP TABLE IF EXISTS server_discord_quests;

-- 5. Recreate server_discord_quests as lean join table
...
-- 6. Create per-member claim tracking table
