...
<div class="space-y-5">
	<div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
		<div class="min-w-0">
			...
			<p class="text-ash-400 text-base">Global catalog. Items appear in every server with the items module enabled.</p>
		</div>
		<button
			onclick={startCreate}
	...
	{:else}
		{#snippet adminCard(item: any)}
			...
			>
				<div class="flex items-start justify-between gap-2">
					<div class="flex min-w-0 items-start gap-2.5">
						...
						<div class="min-w-0">
							...
							<div class="text-ash-500 mt-1 flex flex-wrap gap-1 text-base">
				...
				{#if item.description}<p class="text-ash-400 mt-2 line-clamp-2 text-base">{item.description}</p>{/if}
				<div class="mt-3 flex flex-wrap items-center gap-2">
					<div class="flex items-center gap-1.5" title="Allow members to buy this item">
						...
						<span class="text-ash-400 text-base">Buy</span>
					</div>
					<div class="flex items-center gap-1.5" title="Allow members to use copies they already own">
						...
						<span class="text-ash-400 text-base">Use</span>
					</div>
					<button onclick={() => startEdit(item)} class="bg-ash-700 hover:bg-ash-600 text-ash-200 flex-1 rounded-lg py-1.5 text-base">Edit</button>
					<button
						onclick={() => startGift(item)}
						title="Give to a member"
						aria-label="Give to a member"
						class="rounded-lg bg-teal-900/40 px-3 py-1.5 text-base text-teal-300 hover:bg-teal-900/60"
					>
					...
					<button
						...
						class="rounded-lg bg-red-900/40 px-3 py-1.5 text-base text-red-300 hover:bg-red-900/60"
					>
		...
		<div class="space-y-6">
			{#each adminGroups as group (group.key)}
				<div>
					<h2 class="text-ash-300 mb-2.5 flex items-center gap-2 text-base font-bold uppercase">
						<i class="fas {group.icon}" style="color: var(--effect-{group.key}, var(--effect-default))"></i>{group.label}
						<span class="bg-ash-700 text-ash-400 rounded-full px-2 py-0.5 text-base font-semibold">{group.items.length}</span>
					</h2>
	...
	<div class="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black/50 p-3 lg:p-4" onclick={() => (editing = null)} role="presentation">
		...
		>
			...
			<div class="min-h-0 flex-1 space-y-5 overflow-y-auto px-4 py-5 lg:px-6">
				<div class="space-y-4">
					<div>
						<label for="item-name" class="text-ash-300 mb-1.5 block text-base font-medium">Name</label>
						<input
							id="item-name"
					...
					<div>
						<span class="text-ash-300 mb-1.5 block text-base font-medium">Effect type</span>
						<LabeledSelect id="item-effect" appearance="form-inline" selectClass="w-full" options={effectOptions} bind:value={form.effect_type} />
					</div>

					<div>
						<label for="item-desc" class="text-ash-300 mb-1.5 block text-base font-medium">Description</label>
						<textarea
							id="item-desc"
					...
					<div>
						<label for="item-cost" class="text-ash-300 mb-1.5 block text-base font-medium">Cost (XP)</label>
						<input
							id="item-cost"
							type="number"
				...
				<div class="border-ash-700 bg-ash-900/40 space-y-3 rounded-xl border p-4">
					<p class="text-ash-400 text-base font-semibold uppercase">Effect settings</p>
					{#if form.effect_type === 'steal' || form.effect_type === 'bomb'}
						<div class="flex flex-col gap-3 lg:flex-row lg:flex-wrap *:lg:flex-1 *:lg:basis-0">
							<label class="text-ash-300 text-base"
								>Min %<input
									type="number"
						...
						<p class="text-ash-500 text-base">
					...
					{:else if form.effect_type === 'boost'}
						<div class="flex flex-col gap-3 lg:flex-row lg:flex-wrap *:lg:flex-1 *:lg:basis-0">
							...
							<label class="text-ash-300 text-base"
								...
								/></label
							>
							<label class="text-ash-300 text-base lg:col-span-2"
								>Scope
						...
						<p class="text-ash-500 text-base">Multiplies earned XP for the duration.</p>
					...
					{:else if form.effect_type === 'insurance'}
						...
						<p class="text-ash-500 text-base">Refunds that % of a loss once. Cooldown in minutes (1440 = 1 day).</p>
					{:else if form.effect_type === 'gift'}
						...
						<p class="text-ash-500 text-base">Sends fixed XP to a member, minus the tax (burned).</p>
					{:else if form.effect_type === 'leech'}
						...
						<p class="text-ash-500 text-base">Skims a % of the target's XP while active.</p>
					{:else if form.effect_type === 'bounty'}
						<label class="text-ash-300 text-base"
							...
							/></label
						>
						<p class="text-ash-500 text-base">XP on a member's head, claimed by whoever hits them next.</p>
					...
					{:else}
						<p class="text-ash-500 text-base">No extra settings for this effect type yet.</p>
					{/if}
				</div>

				<div class="border-ash-700 bg-ash-900/40 space-y-3 rounded-xl border p-4">
					<p class="text-ash-400 text-base font-semibold uppercase">Availability (optional, your timezone)</p>
					<div class="flex flex-col gap-3 lg:flex-row lg:flex-wrap *:lg:flex-1 *:lg:basis-0">
						<label class="text-ash-300 text-base">
							From
							<div
							...
							>
								<i class="fas fa-calendar-day text-ash-400 text-base"></i>
								<input
									type="datetime-local"
								...
								{#if form.available_from}
									<button type="button" onclick={() => (form.available_from = '')} aria-label="Clear" class="text-ash-500 hover:text-ash-200 text-base">
						...
						<label class="text-ash-300 text-base">
							...
							>
								...
								{#if form.available_to}
									<button type="button" onclick={() => (form.available_to = '')} aria-label="Clear" class="text-ash-500 hover:text-ash-200 text-base">
										<i class="fas fa-times"></i>
									</button>
								{/if}
							</div>
					...
					<div>
						<p class="text-ash-300 mb-1.5 text-base">Recurring days (your timezone)</p>
						<div class="flex flex-wrap gap-1">
							{#each DAYS as d}
								<button
									type="button"
									onclick={() => toggleDay(d.v)}
									class="rounded-lg px-2.5 py-1 text-base transition-colors {form.cfg.recur_days.includes(d.v)
				...
				{#if isTargeted}<p class="text-ash-500 text-base">
	...
	<div class="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black/50 p-3 lg:p-4" onclick={() => (giftItem = null)} role="presentation">
		...
		>
			...
			<div class="border-ash-700 border-b px-4 py-3 lg:px-6">
				<div class="relative">
					<i class="fas fa-search text-ash-500 absolute top-1/2 left-3 -translate-y-1/2 text-base"></i>
					<input
						type="search"
				...
				<p class="text-ash-500 mt-2 text-base">Only members in servers with the items module enabled are shown. Same person can appear per server.</p>
			</div>
			...
			<div class="min-h-0 flex-1 space-y-4 overflow-y-auto px-4 py-4 lg:px-6">
				...
				{:else}
					{#each giftGroups as group}
						<div>
							<div class="text-ash-400 mb-1.5 flex items-center gap-1.5 text-base font-semibold uppercase">
								...
								{#each group.members as m}
									<div class="bg-ash-700/50 flex items-center gap-3 rounded-lg p-2">
										...
										<div class="min-w-0 flex-1">
											...
											<div class="text-ash-500 text-base"><i class="fas fa-bag-shopping mr-1"></i>{Number(m.inventory_total || 0)} owned</div>
										</div>
										<button
											onclick={() => giveTo(m)}
											disabled={giftingMemberId === m.id}
											class="flex shrink-0 items-center gap-1.5 rounded-lg bg-teal-600 px-3 py-1.5 text-base font-medium text-white transition-colors hover:bg-teal-500 disabled:opacity-50"
										>
