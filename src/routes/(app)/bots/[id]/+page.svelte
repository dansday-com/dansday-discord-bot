<script lang="ts">
	...
	async function handleImageUpload(e: Event) {
		...
		if (uploadedImagePath) {
			await fetch(`/api/bots/${data.bot.id}/delete-embed-image`, {
				method: 'POST',
		...
		const res = await fetch(`/api/bots/${data.bot.id}/upload-embed-image`, {
			method: 'POST',
			credentials: 'include',
			body: formData
		});
	...
	async function sendGlobalEmbed() {
		...
		try {
			...
			const res = await fetch(`/api/bots/${data.bot.id}/send-global-embed`, {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				credentials: 'include',
...
<div class="mx-auto max-w-7xl px-3 py-4 sm:px-4 sm:py-6 lg:px-8 lg:py-8">
	...
	<div class="bg-ash-800 border-ash-700 mb-4 rounded-xl border p-4 sm:mb-6 sm:p-6">
		...
		<p class="text-ash-400 mb-4 text-sm">
			Send an announcement to all servers using this bot. It will be posted in each server's configured <strong>Bot Updates Channel</strong>.
		</p>
