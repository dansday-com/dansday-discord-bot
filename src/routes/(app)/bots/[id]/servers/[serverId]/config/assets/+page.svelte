	...
	async function save() {
		...
		try {
			const res = await fetch(`/api/servers/${data.serverId}/settings`, {
				...
				credentials: 'include',
				body: JSON.stringify({
					component: SERVER_SETTINGS.component.assets,
...
<div class="bg-ash-800 border-ash-700 space-y-5 rounded-xl border p-4 sm:p-6">
	...
	<ConfigToggleRow
		label="Assets module"
		description="When off, the Assets section and all buy/sell actions are disabled for this server."
		labelIconClass="fas fa-chart-line text-sky-400"
		bind:enabled={featureEnabled}
		ariaLabel="Toggle assets module"
	/>
	{#if !featureEnabled}
