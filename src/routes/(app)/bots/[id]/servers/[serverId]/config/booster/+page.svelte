	...
	async function save() {
		...
		try {
			const res = await fetch(`/api/servers/${data.serverId}/settings`, {
				...
				credentials: 'include',
				body: JSON.stringify({ component: SERVER_SETTINGS.component.booster, channels, messages, enabled: featureEnabled })
			});
...
<div class="bg-ash-800 border-ash-700 space-y-5 rounded-xl border p-4 sm:p-6">
	...
	<ConfigToggleRow
		...
		description="When off, boost messages are not sent and related Discord UI is hidden."
		labelIconClass="fas fa-gem text-purple-400"
	...
	<div class="space-y-5 transition-opacity" class:pointer-events-none={!featureEnabled} class:opacity-50={!featureEnabled}>
		...
		<MessageList
			...
			iconAccentMuted="text-purple-400/80"
			values={messages}
