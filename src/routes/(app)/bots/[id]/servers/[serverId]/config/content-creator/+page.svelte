...
<div class="bg-ash-800 border-ash-700 space-y-5 rounded-xl border p-4 lg:p-6">
	...
	<p class="text-ash-400 text-base">Configure content creator admission and TikTok live broadcast behavior.</p>

	<ConfigToggleRow
		label="Content creator module"
		description="When off, content creator admission and related Discord UI are disabled."
		labelIconClass="fas fa-video text-pink-400"
	...
	<div class="space-y-5 transition-opacity" class:pointer-events-none={!featureEnabled} class:opacity-50={!featureEnabled}>
		<div>
			<label class="text-ash-300 mb-1.5 block text-base font-medium"><i class="fas fa-inbox mr-1 text-pink-400"></i>Admission Channel</label>
			<p class="text-ash-500 mb-2 text-base">Staff approval queue channel for pending applications.</p>
			<ChannelPicker channels={data.channels} categories={data.categories} value={admissionChannel} onchange={(id) => (admissionChannel = id)} />
		</div>

		<div>
			<label class="text-ash-300 mb-1.5 block text-base font-medium"><i class="fas fa-bullhorn mr-1 text-pink-400"></i>Target Broadcast Channel</label>
			<p class="text-ash-500 mb-2 text-base">Channel for TikTok LIVE notifications from approved creators.</p>
		...
		<div>
			<label class="text-ash-300 mb-1.5 block text-base font-medium"><i class="fas fa-user-shield mr-1 text-pink-400"></i>Pending Admission Role</label>
			<p class="text-ash-500 mb-2 text-base">Optional role to mention when a new application arrives.</p>
