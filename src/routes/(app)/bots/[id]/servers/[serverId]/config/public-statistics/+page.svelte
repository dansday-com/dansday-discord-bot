...
<div class="bg-ash-800 border-ash-700 space-y-5 rounded-xl border p-4 lg:p-6">
	...
	<div class="space-y-5 transition-opacity" class:pointer-events-none={!enabled} class:opacity-50={!enabled}>
		{#if enabled && data.publicStatsPath}
			<div>
				...
				<div class="bg-ash-900 border-ash-600 flex items-center gap-2 rounded-lg border px-3 py-2">
					<input type="text" readonly value={publicStatsUrl} class="text-ash-100 w-full bg-transparent text-base focus:outline-none" />
					<a class="text-ash-200 hover:text-ash-100 text-base font-medium underline" href={data.publicStatsPath || '#'} target="_blank" rel="noreferrer">
						Open
					</a>
				</div>
			</div>
		...
		<div class="border-ash-700 space-y-5 border-t pt-5">
			<p class="text-ash-300 text-base font-semibold">Account features</p>
			...
			<div class="space-y-3">
				...
				{#if itemsEnabled}
					<div class="pl-1">
						<label class="text-ash-300 mb-1.5 block text-base font-medium"><i class="fas fa-hashtag mr-1 text-teal-400"></i>Item events channel</label>
						<p class="text-ash-500 mb-2 text-base">Where item announcements are posted. If unset, item events are not announced.</p>
			...
			<div class="space-y-3">
				...
				{#if minigamesEnabled}
					<div class="pl-1">
						<label class="text-ash-300 mb-1.5 block text-base font-medium"><i class="fas fa-hashtag mr-1 text-purple-400"></i>Minigame events channel</label>
						<p class="text-ash-500 mb-2 text-base">Where win and loss announcements are posted. If unset, results are not announced.</p>
