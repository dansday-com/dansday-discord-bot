	...
	async function save() {
		...
		try {
			const res = await fetch(`/api/servers/${data.serverId}/settings`, {
				...
				credentials: 'include',
				body: JSON.stringify({ component: 'staff_report_rating', channel, enabled })
			});
...
<div class="bg-ash-800 border border-ash-700 rounded-xl p-4 sm:p-6 space-y-5">
	...
	<div class="flex items-center justify-between">
		<label class="text-xs font-medium text-ash-300">Enable Staff Report & Rating</label>
	...
	<div>
		<label class="text-xs font-medium text-ash-300 block mb-1.5">Report Channel</label>
