	...
	async function save() {
		...
		try {
			const res = await fetch(`/api/servers/${data.serverId}/settings`, {
				...
				credentials: 'include',
				body: JSON.stringify({ component: 'welcomer', channel, messages })
			});
...
<div class="bg-ash-800 border border-ash-700 rounded-xl p-4 sm:p-6 space-y-5">
	<h3 class="text-base font-semibold text-ash-100 flex items-center gap-2">
		<i class="fas fa-hand-wave text-ash-300"></i>Welcomer
	</h3>
	<p class="text-xs text-ash-400">Sent when a new member joins. Supports <code class="text-ash-200">{'{user}'}</code>, <code class="text-ash-200">{'{server}'}</code> placeholders.</p>

	<div>
		<label class="text-xs font-medium text-ash-300 block mb-1.5">Welcome Channel</label>
		<ChannelPicker channels={data.channels} value={channel} onchange={(id) => (channel = id)} />
	</div>

	<MessageList label="Welcome Messages" values={messages} placeholder="Welcome {user} to {server}!" onchange={(v) => (messages = v)} />
