...
import { resolveMemberByCardToken, resolveActiveBotForServer, postBotWebhook } from '$lib/frontend/public/shop/index.js';
...
export const POST: RequestHandler = async ({ params, request }) => {
	...
	const shopSettings = (shopRow as any)?.settings || {};
	if (shopSettings.enabled !== true) {
		return json({ success: false, error: 'Shop is disabled for this server.' }, { status: 403 });
	}

	const body = await request.json().catch(() => null);
	if (!body) return json({ success: false, error: 'Invalid body' }, { status: 400 });
	const { card, item_id, quantity } = body;
	...
	if (webhookResult.status !== 200 || !webhookResult.body?.ok) {
		const err = webhookResult.body?.error || (webhookResult.status === 502 ? 'Could not reach the bot.' : 'Purchase failed.');
